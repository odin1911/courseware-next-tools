# iOS 12 Safari 视频转换工具

## 1. 目标与适用范围

本工具用于转换需要在 iPhone 6s / iOS 12 Safari 中播放的课件 MP4 视频，提供两种模式：

- `format`：只要求兼容格式和连续播放，最长 GOP 为 10 秒并保留场景切换关键帧。
- `seek`：需要从气泡等指定时间开始播放，采用 1 秒 GOP。

目标：

- 兼容旧版 iOS Safari 的硬件解码能力。
- `seek` 模式跳转时，最多回退约 1 秒到最近的独立解码点。
...
| 分辨率 | 最大 1920×1080；等比缩小且不放大小源 | 限制单帧解码负担，避免 4K / Level 5.1，同时避免把 720p 素材无意义放大。 |
| 帧率 | 25 fps，CFR | 与当前课件视频一致；固定帧率让 25 帧 GOP 精确对应 1 秒。 |
| 像素格式 | `yuv420p`，8-bit 4:2:0 | 旧版硬件解码器兼容性最好。 |
| GOP / IDR | `format` 最长 250 帧并允许场景切换关键帧；`seek` 每 25 帧一个 IDR | 10 秒 GOP 限制极端间隔，同时尽量接近编码器默认压缩效率；指定时间播放使用 1 秒快速定位。 |
...
Apple HLS 编码规范建议每 2 秒存在一个 IDR，附录给出的常见配置为每 1～2 秒一个关键帧。但当前资源使用普通 MP4 连续播放而不是 HLS 分片，`format` 模式没有中途跳转需求，因此采用最长 10 秒 GOP，并允许编码器在场景切换时插入关键帧。这样可以限制极端间隔，同时尽量减少额外 I 帧和体积增长。需要从指定时间开始播放时才使用 `seek` 模式，将跳转后的预解码范围限制在约 1 秒内。

该 Apple 规范面向 HLS，普通 MP4 不受其强制约束；这里采用的是其中适合旧 Apple 设备的兼容性基线。

### 3.3 不使用 4K / Level 5.1

已检查的异常样本同时具有 3840×2160、High@5.1、仅首帧为关键帧、`moov` 位于文件末尾等风险因素，不能只凭一次播放失败把原因归到其中某一项。两种模式都会限制为最高 1080p、High@4.1 并启用 `faststart`；只有需要片段跳转时才使用 1 秒 IDR。
...
```bash
./transcode-ios12-video.sh --mode format input.mp4 output.mp4
./transcode-ios12-video.sh --mode seek input.mp4 output.mp4
```

脚本自动生成 `output-f10s.mp4` 或 `output-s1s.mp4`：

- `f10s`：format 模式，最长 10 秒 GOP，允许场景切换关键帧。
- `s1s`：seek 模式，1 秒 GOP。

脚本默认不覆盖已有文件。需要替换时，先明确删除或移动旧输出文件，再重新执行。

批量转换 `videos/` 顶层的所有 MP4、MOV 和 M4V 文件：

```bash
./transcode-videos.sh format
./transcode-videos.sh seek
```

也可以只处理指定文件。例如当前 4 个测试视频按交互需求分组：

```bash
./transcode-videos.sh seek \
  kj-bubble-video-5de411.mp4 \
  kj-bubble-video-47b4f2.mp4

./transcode-videos.sh format \
  kj-bubble-video-b2b875.mp4 \
  kj-bubble-video-6d6d06.mp4
```

转换结果统一写入 `videos/converted/`，例如：

```text
kj-bubble-video-b2b875-f10s.mp4
kj-bubble-video-47b4f2-s1s.mp4
```

已存在的同名输出会跳过；`videos/` 中的测试素材和转换结果均由 Git 忽略。
...
宽高应不超过 1920×1080；小于该尺寸的源视频保持原有画面尺寸。
...
`seek` 模式预期关键帧位于 `0、1、2、3...` 秒，允许因时间基产生不超过一帧的显示误差。`format` 模式的关键帧间隔不超过 10 秒，并可能因场景切换提前出现。
